...
$(document).ready(function () {
    ...
    // manage brand table
    heatsTable = $("#heatsTable").DataTable({
        'ajax': 'php_action/ordenarHeats.php',
        'order': []
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.success == true) {
                    ...
                    heatsTable.ajax.reload(null, false);
...
function editarHeat(brandId) {    
    if(brandId) {
		...
		$.ajax({
			url: 'php_action/ordenarHeatSeleccionado.php',
			type: 'post',
			...
			success:function(response) {
				...
				$('#nombre').val(response.nombreHEAT);
				// setting the brand status value
				$('#vvv').val(response.vueltas);
				// brand id                                 
				$(".editBrandFooter").after('<input type="hidden" name="brandId" id="brandId" value="'+response.codigoHEAT+'" />');
				...
				$('#editBrandForm').unbind('submit').bind('submit', function() {
					...
					var brandName = $('#nombre').val();
					var brandStatus = $('#vvv').val();
					...
					if(brandStatus == "") {
						$("#vvv").after('<p class="text-danger">Este campo es obligatorio</p>');

						$('#vvv').closest('.form-group').addClass('has-error');
					} else {
						// remove error text field
						$("#vvv").find('.text-danger').remove();
						// success out for form 
						$("#vvv").closest('.form-group').addClass('has-success');	  	
					}
...
function eliminarHeat(id)
{
    if (id) {
        ...
        $("#eliminarBodegaModalBtn").unbind('click').bind('click', function () {
            ...
            $.ajax({
                url: 'php_action/eliminarHeat.php',
                type: 'post',
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success == true) {
                        ...
                        heatsTable.ajax.reload(null, true);
                        // remove text-error 
                        $(".text-danger").remove();
